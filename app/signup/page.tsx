...
function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // Redirect to home or pricing page if plan selected
        const redirectUrl = plan ? '/pricing' : '/';
  ...
  const handleGoogleSignup = () => {
    signIn('google', { callbackUrl: plan ? '/pricing' : '/' });
  };

  const handleAppleSignup = () => {
    signIn('apple', { callbackUrl: plan ? '/pricing' : '/' });
  };
        ...
        <div className="bg-[#1a1a1a] rounded-lg border border-gray-800 p-8">
          ...
          {plan && (
            <p className="text-center text-sm text-purple-400 mb-6">
              Selected: {planNames[plan] || 'Free Plan'}
            </p>
          )}

          {!plan && (
            <p className="text-center text-sm text-gray-400 mb-6">
              Join thousands of podcast enthusiasts
            </p>
          )}
          ...
          <div className="space-y-3 mb-6">
            ...
            {/* Apple signup - uncomment when credentials available */}
          </div>
